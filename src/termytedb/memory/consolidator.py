...
def consolidate(repository: Repository, namespace_id: str, *, limit: int = 5, mode: str = "dry-run") -> dict[str, Any]:
    ...
    try:
        for episode_id in episode_ids:
            events = repository.db.execute(
                ...
                WHERE ee.episode_id=? AND ee.namespace_id=? ORDER BY ee.ordinal""", (episode_id, namespace_id)
            ).fetchall()
            ...
            for event in events:
                ...
                for rule in extract({**json.loads(event["payload_json"]), "__termytedb_event_type": event["type"]}):
                    ...
                    proposal = {"episode_id": episode_id, "event_id": event["id"], "kind": candidate.kind,
                                "subject_key": candidate.subject, "statement": candidate.statement}
                    if mode == "dry-run":
                        repository.record_consolidation_proposal(namespace_id, run_id, candidate.kind, candidate.subject,
                                                                 candidate.statement, [event["id"]], "proposed", "dry_run")
                    ...
                    try:
                        ...
                        if candidate.kind == "procedure":
                            repository.upsert_procedure(
                                namespace_id, candidate.statement, str(event["type"]), [], [candidate.statement],
                                candidate.statement, candidate.statement, [], True,
                                [(str(event["id"]), candidate.statement)],
                            )
                        repository.record_consolidation_proposal(namespace_id, run_id, candidate.kind, candidate.subject,
                                                                 candidate.statement, [event["id"]], "accepted", action,
                                                                 memory_id, version_id)
                        proposals.append({**proposal, "action": action, "memory_id": memory_id})
                        accepted += 1
                    except Exception as exc:
                        repository.record_consolidation_proposal(namespace_id, run_id, candidate.kind, candidate.subject,
                                                                 candidate.statement, [event["id"]], "rejected", type(exc).__name__)
                        rejected += 1
    ...
    return {"run_id": run_id, "mode": mode, "episode_count": len(episode_ids), "proposal_count": len(proposals),
            "accepted": accepted, "rejected": rejected, "proposals": proposals}

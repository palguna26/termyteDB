...
def score_observation(text: str, *, repeated: float = 0.0, task_relevance: float = 0.5) -> EncodingDecision:
    ...
    raw = (0.20 * emphasis + 0.20 * novelty + 0.18 * surprise + 0.14 * task_relevance
           + 0.10 * min(1.0, repeated) + 0.08 * outcome + 0.12 * correction + 0.10 * future_use
           - 0.15 * privacy)
    score = min(1.0, max(0.0, raw))
    reasons = [name for name, value in (
        ("explicit_emphasis", emphasis), ("novel", novelty), ("surprising", surprise),
        ("task_relevant", task_relevance), ("outcome", outcome), ("correction", correction),
        ("future_use", future_use), ("privacy_sensitive", privacy),
    ) if value > 0]
    return EncodingDecision(
        score, novelty, surprise, task_relevance, min(1.0, repeated), outcome,
        correction, future_use, privacy, ",".join(reasons) or "low_signal",
    )

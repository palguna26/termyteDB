...
from termytedb.retrieval.retrieval import search_atoms
...
def run(path: Path, top_k: int, database_path: Path | None = None, batch_size: int = 4) -> dict[str, object]:
    ...
    try:
        ...
        for question in questions:
            ...
            for session_index, session in enumerate(question.get("haystack_sessions", [])):
                ...
                content = "\n".join(str(turn.get("content", "")) for turn in session if isinstance(turn, dict) and turn.get("content"))
                if content:
                    atoms.append(L1Atom(f"{question_id}:{session_id}", session_id, content, timestamp, "user"))
        inserted = insert_atoms(db, atoms)
        indexed = index_atom_embeddings(db, FastEmbedProvider(), batch_size=batch_size)
        ...
        for question in questions:
            ...
            hits = search_atoms(db, str(question["question"]), limit=max(top_k * 8, 40))
...
def main() -> int:
    ...
    result = run(args.dataset, args.top_k, args.database, args.batch_size)

...
Candidate sources in local V1 are SQLite FTS5 and deterministic rebuildable embeddings. Metadata and validity predicates apply in SQL before candidates leave the scope. Graph expansion is not required. The current local score is explainable: lexical score 0.6 and vector score 0.4. These are starting constants, not trained weights; benchmark data may change them.
